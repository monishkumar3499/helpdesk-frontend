            ...
            {tickets.slice(0, 5).map(t => (
              <div key={t.id} className="flex items-center justify-between bg-white rounded-lg border border-slate-100 px-4 py-3">
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${t.status === "OPEN" ? "bg-blue-100 text-blue-700" :
                      t.status === "IN_PROGRESS" ? "bg-yellow-100 text-yellow-700" :
                        t.status === "RESOLVED" ? "bg-green-100 text-green-700" :
                          "bg-red-100 text-red-700"
                    }`}>{t.status.replace("_", " ")}</span>
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${t.priority === "CRITICAL" ? "bg-red-100 text-red-700" :
            ...
            {commentedTickets.length === 0 ? (
              ...
              <div className="space-y-3">
                {commentedTickets.map(t => (
                  <div key={t.id} className={`p-3 rounded-lg border-l-2 ${t.status === "REJECTED" ? "bg-red-50 border-red-400" : "bg-green-50 border-green-400"
                    }`}>
                    <div className="flex items-center justify-between mb-1">
                      <p className="text-sm font-semibold text-slate-800">{t.employeeName}</p>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${t.status === "REJECTED" ? "bg-red-100 text-red-700" :
                          t.status === "RESOLVED" ? "bg-green-100 text-green-700" :
