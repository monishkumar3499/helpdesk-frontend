...
export default function ITLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!loading) {
      if (!isAuthenticated || !user) {
        router.push("/login")
      } else if (!["IT_ADMIN", "IT_SUPPORT", "ADMIN"].includes(user.role)) {
        router.push("/HR")
      }
    }
  ...
  const userRole = user?.role || "IT_SUPPORT"
  ...
  const navItems = useMemo(() => {
    ...
    if (!isITAdmin(userRole)) return common
        ...
        <header className="h-14 border-b flex items-center justify-between px-6 bg-white">
          <div className="flex items-center gap-3">
            ...
            <Badge className="it-pill bg-slate-100 text-slate-700">
              {isITAdmin(userRole) ? "IT Admin" : "IT Support"}
            </Badge>
