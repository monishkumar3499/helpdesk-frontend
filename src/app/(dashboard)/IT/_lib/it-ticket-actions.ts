...
import { getMemberLoad, normalizeTicket, toArrayResponse, type ITTicket, type ITUser } from "./it-shared"
...
export async function refreshTickets(setTickets: (tickets: ITTicket[]) => void) {
  try {
    ...
    setTickets(toArrayResponse<unknown>(fresh).map(normalizeTicket))
  } catch {
    setTickets([])
  }
}
